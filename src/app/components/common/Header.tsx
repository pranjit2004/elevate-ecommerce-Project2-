  ...
  return (
    <header
      className={`sticky top-0 z-50 w-full transition-all duration-300 backdrop-blur-md bg-white/80 dark:bg-neutral-950/80 ${
        isScrolled ? 'border-b border-neutral-200 dark:border-neutral-800 shadow-sm' : 'border-b border-transparent'
      }`}
    >
          ...
          <div className="shrink-0 flex items-center">
            <Link href="/" className="flex items-center gap-2">
              <div className="h-8 w-8 rounded-lg bg-black dark:bg-white flex items-center justify-center">
                <span className="text-white dark:text-black font-bold text-xl leading-none">E</span>
              </div>
          ...
          <div className="flex items-center justify-end gap-4 md:gap-6 shrink-0">
            ...
            <div className="hidden md:flex items-center border-l border-neutral-200 dark:border-neutral-800 pl-6">
              {status === 'loading' ? (
                ...
                <Link href="/dashboard" className="h-8 w-8 rounded-full overflow-hidden border border-neutral-200 dark:border-neutral-700 hover:ring-2 hover:ring-black dark:hover:ring-white transition-all">
                  <img src={session?.user?.image || ''} alt="Profile" className="h-full w-full object-cover" />
                </Link>
              ) : (
                <div className="flex items-center gap-4">
                  <Link href="/api/auth/signin" className="text-sm font-medium text-neutral-600 hover:text-black dark:text-neutral-400 dark:hover:text-white transition-colors">
                    Log in
                  </Link>
                  <Link href="/api/auth/signin" className="text-sm font-medium bg-black dark:bg-white text-white dark:text-black px-4 py-2 rounded-full hover:bg-neutral-800 dark:hover:bg-neutral-200 transition-colors">
                    Sign up
                  </Link>
                </div>
              )}
      ...
      <div
        className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${
          isMobileMenuOpen ? 'max-h-96 border-t border-neutral-200 dark:border-neutral-800 opacity-100' : 'max-h-0 opacity-0'
